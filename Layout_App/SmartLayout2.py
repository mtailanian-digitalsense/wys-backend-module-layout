...
class Module:
    def __init__(self, x, y, rotation, name, identificator, width_value, height_value):
        ...
        self._box = box(x - width_value / 2, y - height_value / 2, x + width_value / 2, y + height_value / 2)
...
def Smart_Layout(dictionary, POP_SIZE, GENERATIONS, viz = False, viz_period = 10):
            ...
            if random.random() < indpb:
                ...
                if not planta.contains(i.box):
                    if i.y > maxy:
                        ...
                        i.y = miny + i.height/2
            i.box = box(i.x - i.width / 2, i.y - i.height / 2, i.x + i.width / 2, i.y + i.height / 2)
            if random.random() < indpb:
    ...
    def distancebtwmods(ind):
        ...
        for i in ind:
            ...
            for j in ind:
                if i != j:
                    distances.append([j.name, i_box.distance(j.box)])
    ...
    def modtoareas(As, ind):
        ...
        for i in ind:
            ...
            for j in ind:
                if i != j:
                    mod2mod_distances.append([j.name, i_box.distance(j.box)])

            #print('list of', i, 'module')
            #for d in distances:
            #    print(d)
            min_mod2mod_dist = min_dist_to_area(mod2mod_distances)
            #print('CONSOLIDATED LIST:')
            #print('list of', i, 'module')
            for d in min_mod2mod_dist:
                w = restrictions.mod2mod(restrictions.module_dictionary, restrictions.mod2mod_matrix,
                                            i.name, d[0])
                if w != 0:
                    a -= d[1] * w
    ...
    def feasible(ind):  # Need too add if boxes collide
        ...
        for i in range(n_ind):
            i_box = ind[i].box
            if not planta.contains(i_box):
                return False
            for j in range(i + 1, n_ind):
                if i_box.intersects(ind[j].box):
                    return False
        return True
    ...
    def feas_distance(ind):
        ...
        for i in range(n_ind):
            ...
            area += (ind[i].box.area - (ind[i].box.intersection(planta)).area)
            for j in range(i + 1, n_ind):
                area += (i_box.intersection(ind[j].box).area)
